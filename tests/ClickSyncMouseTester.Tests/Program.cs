...
internal static class Program
{
    ...
    private static int Main()
    {
        ...
        (string Name, Action Run)[] tests =
        {
            ...
            ("Angle calibration reuses historical trace snapshots", AngleCalibrationReusesHistoricalTraceSnapshots),
            ...
            ("Brand logo key resolves text logo", BrandLogoKeyResolvesTextLogo),
            ("Brand logo key resolves Logi vector logo", BrandLogoKeyResolvesLogiVectorLogo),
            ("Brand logo key resolves Razer vector logo", BrandLogoKeyResolvesRazerVectorLogo),
            ("Brand logo key resolves preset vector logo", BrandLogoKeyResolvesPresetVectorLogo),
            ("Brand logo key resolves ATK mark and letters", BrandLogoKeyResolvesAtkMarkAndLetters),
            ("Brand logo key resolves Lamzu letters", BrandLogoKeyResolvesLamzuLetters),
            ("Brand logo key resolves Maicong letters", BrandLogoKeyResolvesMaicongLetters),
            ("Brand logo key resolves Rapoo letters", BrandLogoKeyResolvesRapooLetters),
            ("Brand logo key resolves Vaxee letters", BrandLogoKeyResolvesVaxeeLetters),
            ("Brand logo key resolves Zowie vector logo", BrandLogoKeyResolvesZowieVectorLogo),
            ("Brand logo key resolves ROG vector logo", BrandLogoKeyResolvesRogVectorLogo),
            ("Brand logo key falls back when unknown", BrandLogoKeyFallsBackWhenUnknown),
            ("Brand logo recursive selection returns no geometry", BrandLogoRecursiveSelectionReturnsNoGeometry),
            ("Device brand logo matcher resolves known brands", DeviceBrandLogoMatcherResolvesKnownBrands),
            ("Device brand logo matcher resolves vendor fallbacks", DeviceBrandLogoMatcherResolvesVendorFallbacks),
            ("Device brand logo matcher falls back to Nuit", DeviceBrandLogoMatcherFallsBackToNuit),
            ("Device selection coordinator prefers manual selection", DeviceSelectionCoordinatorPrefersManualSelection),
            ("Device view models synchronize explicit manual selection", DeviceViewModelsSynchronizeExplicitManualSelection),
            ("Navigation brand device resolver uses global preselection", NavigationBrandDeviceResolverUsesGlobalPreselection),
            ("Navigation brand device resolver prefers synchronized manual selection", NavigationBrandDeviceResolverPrefersSynchronizedManualSelection),
            ("Navigation brand logo service resolves resources with fallback", NavigationBrandLogoServiceResolvesResourcesWithFallback),
    ...
    private static void AngleCalibrationReusesHistoricalTraceSnapshots()
    {
        AngleCalibrationEngine engine = new AngleCalibrationEngine();
        engine.SetLocked(isLocked: true, 0.0);
        long sequence = 1L;
        double timestampMs = 0.0;
        for (int packetIndex = 0; packetIndex < 12; packetIndex++)
        {
            engine.PushPacket(CreatePacket("angle", timestampMs, sequence++, 25, 0));
            timestampMs += 12.0;
        }
        for (int packetIndex = 0; packetIndex < 12; packetIndex++)
        {
        ...
        AngleCalibrationRenderFrame firstFrame = engine.CreateRenderFrame(timestampMs);
        AngleCalibrationRenderFrame secondFrame = engine.CreateRenderFrame(timestampMs + 16.0);
        GreaterThanOrEqual(firstFrame.TraceStrokes.Count, 2, "first trace stroke count");
        GreaterThanOrEqual(secondFrame.TraceStrokes.Count, 2, "second trace stroke count");
        SameReference(firstFrame.TraceStrokes[0], secondFrame.TraceStrokes[0], "historical angle trace stroke snapshot");
    }
    ...
    private static void BrandLogoKeyResolvesTextLogo()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
            SelectedKey = "nuit",
            FallbackKey = "crdrako"
        }, pixelsPerDip: 1.0);

        Equal(4, elements.Count, "nuit glyph element count");
    }

    private static void BrandLogoKeyResolvesLogiVectorLogo()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(2, elements.Count, "logi mark plus wordmark element count");
        Equal(true, elements[1].Bounds.Width > elements[1].Bounds.Height, "logitech wordmark remains horizontal");
    }

    private static void BrandLogoKeyResolvesRazerVectorLogo()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(2, elements.Count, "razer mark plus wordmark element count");
        Equal(true, elements[1].Bounds.Width > elements[1].Bounds.Height, "razer wordmark remains horizontal");
    }

    private static void BrandLogoKeyResolvesPresetVectorLogo()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(7, elements.Count, "crdrako vector element count");
    }

    private static void BrandLogoKeyResolvesAtkMarkAndLetters()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(4, elements.Count, "atk mark plus A/T/K element count");
        Equal(true, elements[1].Bounds.Height > elements[1].Geometry.Bounds.Height * 2.5, "atk letter preserves original mark-to-text scale");
    }

    private static void BrandLogoKeyResolvesLamzuLetters()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(5, elements.Count, "lamzu letter element count");
        Equal(true, elements[0].Bounds.Width > elements[0].Geometry.Bounds.Width, "lamzu letters preserve shared wordmark proportions");
    }

    private static void BrandLogoKeyResolvesMaicongLetters()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(6, elements.Count, "maicong letter element count");
        Equal(true, elements[2].Bounds.Width > elements[2].Geometry.Bounds.Width, "maicong letters preserve source wordmark spacing");
    }

    private static void BrandLogoKeyResolvesRapooLetters()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(5, elements.Count, "rapoo letter element count");
        Equal(true, elements[0].Bounds.Width > elements[0].Geometry.Bounds.Width, "rapoo letters preserve source wordmark spacing");
    }

    private static void BrandLogoKeyResolvesVaxeeLetters()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(5, elements.Count, "vaxee letter element count");
        Equal(true, elements[0].Bounds.Width > 0.0, "vaxee text letter has usable bounds");
    }

    private static void BrandLogoKeyResolvesZowieVectorLogo()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(2, elements.Count, "zowie mark plus wordmark element count");
        Equal(true, elements[1].Bounds.Width > elements[1].Bounds.Height, "zowie wordmark remains horizontal");
    }

    private static void BrandLogoKeyResolvesRogVectorLogo()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(2, elements.Count, "rog mark plus wordmark element count");
        Equal(true, elements[1].Bounds.Width > elements[1].Bounds.Height, "rog wordmark remains horizontal");
    }

    private static void BrandLogoKeyFallsBackWhenUnknown()
    {
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(CreateBrandLogoResources());
        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(new SelectedBrandLogoDefinition
        {
        ...
        Equal(7, elements.Count, "fallback crdrako vector element count");
    }

    private static void BrandLogoRecursiveSelectionReturnsNoGeometry()
    {
        Dictionary<string, object> resources = new Dictionary<string, object>(StringComparer.Ordinal);
        SelectedBrandLogoDefinition recursiveDefinition = new SelectedBrandLogoDefinition
        {
            SelectedKey = "recursive",
            FallbackKey = "recursive"
        };
        resources["BrandLogo.recursive"] = recursiveDefinition;
        BrandLogoGeometryProviderRegistry registry = CreateBrandLogoRegistry(resources);

        IReadOnlyList<BrandLogoGeometryElement> elements = registry.Create(recursiveDefinition, pixelsPerDip: 1.0);

        Equal(0, elements.Count, "recursive selection element count");
    }

    private static void DeviceBrandLogoMatcherResolvesKnownBrands()
    {
        DeviceBrandLogoMatcher matcher = new DeviceBrandLogoMatcher();

        Equal("atk", matcher.ResolveBrandKey(CreateBrandTestDevice("ATK Blazing Sky X1")), "atk brand key");
        Equal("rapoo", matcher.ResolveBrandKey(CreateBrandTestDevice("Rapoo VT9 Pro")), "rapoo brand key");
        Equal("razer", matcher.ResolveBrandKey(CreateBrandTestDevice("Razer Viper V3 Pro")), "razer brand key");
        Equal("crdrako", matcher.ResolveBrandKey(CreateBrandTestDevice("CRDRAKO Blade")), "crdrako brand key");
        Equal("logi", matcher.ResolveBrandKey(CreateBrandTestDevice("Logitech G Pro X Superlight")), "logitech brand key");
        Equal("lamzu", matcher.ResolveBrandKey(CreateBrandTestDevice("LAMZU Maya X")), "lamzu brand key");
        Equal("maicong", matcher.ResolveBrandKey(CreateBrandTestDevice("Mai Cong A5 Pro")), "maicong latin brand key");
        Equal("maicong", matcher.ResolveBrandKey(CreateBrandTestDevice("MCHOSE A5 Pro")), "maicong mchose brand key");
        Equal("maicong", matcher.ResolveBrandKey(CreateBrandTestDevice("\u8fc8\u4ece A5 Pro")), "maicong chinese brand key");
        Equal("zowie", matcher.ResolveBrandKey(CreateBrandTestDevice("BenQ ZOWIE EC2-CW")), "zowie brand key");
        Equal("rog", matcher.ResolveBrandKey(CreateBrandTestDevice("ASUS ROG Harpe Ace")), "rog brand key");
        Equal("vaxee", matcher.ResolveBrandKey(CreateBrandTestDevice("VAXEE XE Wireless")), "vaxee brand key");
    }

    private static void DeviceBrandLogoMatcherResolvesVendorFallbacks()
    {
        DeviceBrandLogoMatcher matcher = new DeviceBrandLogoMatcher();

        Equal("atk", matcher.ResolveBrandKey(CreateBrandTestDevice("Wireless Mouse 8K Dongle-L", 0x373B, 0x0001)), "atk generic dongle vendor fallback");
        Equal("logi", matcher.ResolveBrandKey(CreateBrandTestDevice("Wireless Receiver", 0x046D, 0x0001)), "logitech generic receiver vendor fallback");
        Equal("razer", matcher.ResolveBrandKey(CreateBrandTestDevice("USB Receiver", 0x1532, 0x0001)), "razer generic receiver vendor fallback");
        Equal("rapoo", matcher.ResolveBrandKey(CreateBrandTestDevice("Wireless Receiver", 0x24AE, 0x0001)), "rapoo generic receiver vendor fallback");
        Equal("lamzu", matcher.ResolveBrandKey(CreateBrandTestDevice("Wireless Receiver", 0x37B0, 0x0001)), "lamzu generic receiver vendor fallback");
        Equal("zowie", matcher.ResolveBrandKey(CreateBrandTestDevice("USB Receiver", 0x2345, 0x0001)), "zowie generic receiver vendor fallback");
        Equal("nuit", matcher.ResolveBrandKey(CreateBrandTestDevice("Wireless Mouse 8K Dongle-L", 0x3710, 0x0001)), "unknown generic dongle vendor fallback");
        Equal("rapoo", matcher.ResolveBrandKey(CreateBrandTestDevice("Rapoo VT9 Pro", 0x373B, 0x0001)), "name match wins over vendor fallback");
    }

    private static void DeviceBrandLogoMatcherFallsBackToNuit()
    {
        DeviceBrandLogoMatcher matcher = new DeviceBrandLogoMatcher();

        Equal("nuit", matcher.ResolveBrandKey(null), "null device fallback brand key");
        Equal("nuit", matcher.ResolveBrandKey(CreateBrandTestDevice("HID Mouse")), "unknown device fallback brand key");
    }

    private static void DeviceSelectionCoordinatorPrefersManualSelection()
    {
        RawMouseDeviceInfo atkDevice = CreateBrandTestDevice("ATK Blazing Sky X1");
        RawMouseDeviceInfo rapooDevice = CreateBrandTestDevice("Rapoo VT9 Pro");
        DeviceSelectionCoordinator coordinator = new DeviceSelectionCoordinator();
        coordinator.CommitManualSelection(rapooDevice);

        RawMouseDeviceInfo selectedDevice = coordinator.ResolveSelectionAfterRefresh(new[] { atkDevice, rapooDevice }, atkDevice.DeviceId);

        Equal(rapooDevice.DeviceId, selectedDevice?.DeviceId, "manual selection wins over previous page-local selection");
    }

    [SupportedOSPlatform("windows")]
    private static void DeviceViewModelsSynchronizeExplicitManualSelection()
    {
        InitializeLocalizationForViewModelTests();
        RawMouseDeviceInfo atkDevice = CreateBrandTestDevice("ATK Blazing Sky X1");
        RawMouseDeviceInfo rapooDevice = CreateBrandTestDevice("Rapoo VT9 Pro");
        TestRawInputBroker broker = new TestRawInputBroker(new[] { atkDevice, rapooDevice });
        DeviceSelectionCoordinator coordinator = new DeviceSelectionCoordinator();
        using MainWindowViewModel pollingViewModel = new MainWindowViewModel(broker, coordinator);
        using MousePerformancePageViewModel performanceViewModel = new MousePerformancePageViewModel(broker, new InMemoryMousePerformancePreferencesStore(), coordinator);

        pollingViewModel.SelectedDevice = rapooDevice;
        Equal(atkDevice.DeviceId, performanceViewModel.SelectedDevice?.DeviceId, "plain setter does not broadcast manual intent");

        pollingViewModel.CommitManualDeviceSelection();
        Equal(rapooDevice.DeviceId, performanceViewModel.SelectedDevice?.DeviceId, "performance page follows polling page manual selection");

        performanceViewModel.SelectedDevice = atkDevice;
        performanceViewModel.CommitManualDeviceSelection();
        Equal(atkDevice.DeviceId, pollingViewModel.SelectedDevice?.DeviceId, "polling page follows performance page manual selection");
    }

    private static void InitializeLocalizationForViewModelTests()
    {
        LocalizationManager localization = LocalizationManager.Instance;
        SetPrivateField(localization, "_initialized", true);
        SetPrivateField(localization, "_currentLanguage", new LanguageOption("en-US", "English", "English"));
        Thread.CurrentThread.CurrentCulture = CultureInfo.GetCultureInfo("en-US");
        Thread.CurrentThread.CurrentUICulture = CultureInfo.GetCultureInfo("en-US");
    }

    private static void NavigationBrandDeviceResolverUsesGlobalPreselection()
    {
        RawMouseDeviceInfo atkDevice = CreateBrandTestDevice("ATK Blazing Sky X1");
        RawMouseDeviceInfo rapooDevice = CreateBrandTestDevice("Rapoo VT9 Pro");
        FakeRawInputDeviceCatalog catalog = new FakeRawInputDeviceCatalog(
            new[] { atkDevice, rapooDevice },
            new Dictionary<string, RawMouseEndpointActivitySnapshot>(StringComparer.OrdinalIgnoreCase)
            {
                [rapooDevice.DeviceId] = CreateMotionCapableActivitySnapshot()
            });

        RawMouseDeviceInfo preferredDevice = new NavigationBrandDeviceResolver(catalog).ResolvePreferredDevice();

        Equal(1, catalog.RefreshRequestCount, "navigation resolver refreshes device catalog before resolving");
        Equal(rapooDevice.DeviceId, preferredDevice?.DeviceId, "global preselected device follows selectable motion policy");
    }

    private static void NavigationBrandDeviceResolverPrefersSynchronizedManualSelection()
    {
        RawMouseDeviceInfo atkDevice = CreateBrandTestDevice("ATK Blazing Sky X1");
        RawMouseDeviceInfo rapooDevice = CreateBrandTestDevice("Rapoo VT9 Pro");
        FakeRawInputDeviceCatalog catalog = new FakeRawInputDeviceCatalog(
            ...
            new Dictionary<string, RawMouseEndpointActivitySnapshot>(StringComparer.OrdinalIgnoreCase)
            {
                [atkDevice.DeviceId] = CreateMotionCapableActivitySnapshot(),
                [rapooDevice.DeviceId] = CreateMotionCapableActivitySnapshot()
            });
        DeviceSelectionCoordinator coordinator = new DeviceSelectionCoordinator();
        coordinator.CommitManualSelection(rapooDevice);

        RawMouseDeviceInfo preferredDevice = new NavigationBrandDeviceResolver(catalog, coordinator).ResolvePreferredDevice();

        Equal(rapooDevice.DeviceId, preferredDevice?.DeviceId, "navigation logo device follows synchronized manual selection");
    }

    private static void NavigationBrandLogoServiceResolvesResourcesWithFallback()
    {
        Dictionary<string, object> resources = CreateBrandLogoResources();
        NavigationBrandLogoService service = new NavigationBrandLogoService(
            new DeviceBrandLogoMatcher(),
            key => resources.TryGetValue(key, out object resource) ? resource : null);

        BrandLogoDefinition rapooDefinition = service.ResolveLogoDefinition(CreateBrandTestDevice("Rapoo VT9 Pro"));
        BrandLogoDefinition fallbackDefinition = service.ResolveLogoDefinition(CreateBrandTestDevice("Unknown Mouse"));

        Equal(true, rapooDefinition is PresetVectorBrandLogoDefinition rapooPreset && rapooPreset.PresetKey == "rapoo", "rapoo resource definition");
        Equal(true, fallbackDefinition is TextBrandLogoDefinition nuitText && nuitText.Text == "NUIT", "unknown resource falls back to nuit");

        resources.Remove("BrandLogo.rapoo");
        BrandLogoDefinition missingBrandDefinition = service.ResolveLogoDefinition(CreateBrandTestDevice("Rapoo VT9 Pro"));
        Equal(true, missingBrandDefinition is TextBrandLogoDefinition missingBrandFallback && missingBrandFallback.Text == "NUIT", "missing brand resource falls back to nuit");
    }
    ...
    private static void SetPrivateField(object instance, string fieldName, object value)
    {
        if (instance == null)
        {
            throw new InvalidOperationException($"missing instance for {fieldName}");
        }
    ...
    private static RawMouseDeviceInfo CreateBrandTestDevice(string displayName)
    {
        return CreateBrandTestDevice(displayName, 0x0001, 0x0001);
    }

    private static RawMouseDeviceInfo CreateBrandTestDevice(string displayName, int vendorId, int productId)
    {
        return new RawMouseDeviceInfo(
            $"\\\\?\\hid#vid_{vendorId:X4}&pid_{productId:X4}#{Guid.NewGuid():N}#{{378de44c-56ef-11d1-bc8c-00a0c91405dd}}",
            displayName,
            vendorId,
            productId,
            buttonCount: 5,
    ...
    private static BrandLogoGeometryProviderRegistry CreateBrandLogoRegistry(Dictionary<string, object> resources)
    {
        return new BrandLogoGeometryProviderRegistry(new IBrandLogoGeometryProvider[]
        {
            new TextBrandLogoGeometryProvider(),
            new PresetVectorBrandLogoGeometryProvider()
        }, key => resources.TryGetValue(key, out object value) ? value : null);
    }

    private static Dictionary<string, object> CreateBrandLogoResources()
    {
        GeometryGroup crdrakoGeometry = new GeometryGroup { FillRule = FillRule.Nonzero };
        for (int index = 0; index < 7; index++)
        {
            crdrakoGeometry.Children.Add(new RectangleGeometry(new System.Windows.Rect(index * 12.0, 0.0, 8.0, 16.0)));
        }

        BrandLogoVectorPreset atkPreset = new BrandLogoVectorPreset();
        atkPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 30.0, 32.0)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 30.0, 32.0)
        });
        atkPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(34.0, 11.0, 12.0, 10.0)),
            LayoutBounds = new System.Windows.Rect(34.0, 0.0, 12.0, 32.0)
        });
        atkPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(44.0, 11.0, 11.0, 10.0)),
            LayoutBounds = new System.Windows.Rect(44.0, 0.0, 11.0, 32.0)
        });
        atkPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(55.0, 11.0, 11.0, 10.0)),
            LayoutBounds = new System.Windows.Rect(55.0, 0.0, 11.0, 32.0)
        });

        BrandLogoVectorPreset lamzuPreset = new BrandLogoVectorPreset();
        for (int index = 0; index < 5; index++)
        {
            lamzuPreset.Elements.Add(new BrandLogoVectorElement
            {
                Geometry = new RectangleGeometry(new System.Windows.Rect(64.0, 12.0, 256.0, 328.0)),
                LayoutBounds = new System.Windows.Rect(0.0, 0.0, 382.0, 341.0)
            });
        }

        BrandLogoVectorPreset maicongPreset = new BrandLogoVectorPreset();
        for (int index = 0; index < 6; index++)
        {
            maicongPreset.Elements.Add(new BrandLogoVectorElement
            {
                Geometry = new RectangleGeometry(new System.Windows.Rect(index * 20.0 + 4.0, 6.0, 14.0, 20.0)),
                LayoutBounds = new System.Windows.Rect(index * 20.0, 0.0, 20.0, 32.0)
            });
        }

        BrandLogoVectorPreset rapooPreset = new BrandLogoVectorPreset();
        for (int index = 0; index < 5; index++)
        {
            rapooPreset.Elements.Add(new BrandLogoVectorElement
            {
                Geometry = new RectangleGeometry(new System.Windows.Rect(index * 18.0 + 4.0, 0.0, 12.0, 19.0)),
                LayoutBounds = new System.Windows.Rect(index * 18.0, 0.0, 18.0, 26.0)
            });
        }

        BrandLogoVectorPreset zowiePreset = new BrandLogoVectorPreset();
        zowiePreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 23.74, 22.12)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 23.74, 22.12)
        });
        zowiePreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 63.49, 13.42)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 63.49, 13.42)
        });

        BrandLogoVectorPreset rogPreset = new BrandLogoVectorPreset();
        rogPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 68.0, 37.0)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 68.0, 37.0)
        });
        rogPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 125.0, 26.6)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 125.0, 26.6)
        });

        BrandLogoVectorPreset razerPreset = new BrandLogoVectorPreset();
        razerPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 196.01, 196.9)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 196.01, 196.9)
        });
        razerPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 196.01, 120.0)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 196.01, 120.0)
        });

        BrandLogoVectorPreset logiPreset = new BrandLogoVectorPreset();
        logiPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 28.96, 30.76)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 28.96, 30.76)
        });
        logiPreset.Elements.Add(new BrandLogoVectorElement
        {
            Geometry = new RectangleGeometry(new System.Windows.Rect(0.0, 0.0, 84.95, 31.48)),
            LayoutBounds = new System.Windows.Rect(0.0, 0.0, 84.95, 31.48)
        });

        return new Dictionary<string, object>(StringComparer.Ordinal)
        {
            ["BrandLogo.nuit"] = new TextBrandLogoDefinition { Text = "NUIT" },
            ["BrandLogo.logi"] = new PresetVectorBrandLogoDefinition { PresetKey = "logi" },
            ["BrandLogo.razer"] = new PresetVectorBrandLogoDefinition { PresetKey = "razer" },
            ["BrandLogo.crdrako"] = new PresetVectorBrandLogoDefinition { PresetKey = "crdrako", SplitGeometryGroup = true },
            ["BrandLogo.atk"] = new PresetVectorBrandLogoDefinition { PresetKey = "atk", SplitGeometryGroup = true },
            ["BrandLogo.lamzu"] = new PresetVectorBrandLogoDefinition { PresetKey = "lamzu", SplitGeometryGroup = true },
            ["BrandLogo.maicong"] = new PresetVectorBrandLogoDefinition { PresetKey = "maicong", SplitGeometryGroup = true },
            ["BrandLogo.rapoo"] = new PresetVectorBrandLogoDefinition { PresetKey = "rapoo", SplitGeometryGroup = true },
            ["BrandLogo.zowie"] = new PresetVectorBrandLogoDefinition { PresetKey = "zowie" },
            ["BrandLogo.rog"] = new PresetVectorBrandLogoDefinition { PresetKey = "rog" },
            ["BrandLogo.vaxee"] = new TextBrandLogoDefinition
            {
                Text = "vaxee",
                FontFamily = new FontFamily("Century Gothic")
            },
            ["crdrako"] = crdrakoGeometry,
            ["atk"] = atkPreset,
            ["lamzu"] = lamzuPreset,
            ["maicong"] = maicongPreset,
            ["rapoo"] = rapooPreset,
            ["zowie"] = zowiePreset,
            ["rog"] = rogPreset,
            ["logi"] = logiPreset,
    ...
    private sealed class FakeRawInputDeviceCatalog : IRawInputDeviceCatalog
    {
        private readonly IReadOnlyList<RawMouseDeviceInfo> _devices;
        ...
        public FakeRawInputDeviceCatalog(IReadOnlyList<RawMouseDeviceInfo> devices, IReadOnlyDictionary<string, RawMouseEndpointActivitySnapshot> activitySnapshots)
        {
            _devices = devices ?? Array.Empty<RawMouseDeviceInfo>();
            _activitySnapshots = activitySnapshots ?? new Dictionary<string, RawMouseEndpointActivitySnapshot>(StringComparer.OrdinalIgnoreCase);
        }
    ...
    private sealed class TestRawInputBroker : IRawInputBroker
    {
        private readonly IReadOnlyList<RawMouseDeviceInfo> _devices;
        ...
        public TestRawInputBroker(IReadOnlyList<RawMouseDeviceInfo> devices)
        {
            _devices = devices ?? Array.Empty<RawMouseDeviceInfo>();
            Dictionary<string, RawMouseEndpointActivitySnapshot> activitySnapshots = new Dictionary<string, RawMouseEndpointActivitySnapshot>(StringComparer.OrdinalIgnoreCase);
            foreach (RawMouseDeviceInfo device in _devices)
            {
                if (device != null && !string.IsNullOrWhiteSpace(device.DeviceId))
                {
                    activitySnapshots[device.DeviceId] = CreateMotionCapableActivitySnapshot();
                }
            }
    ...
    private sealed class InMemoryMousePerformancePreferencesStore : IMousePerformancePreferencesStore
    {
        private double? _lastCpi = 800.0;

        private MousePerformancePlotType _plotType = MousePerformancePlotType.XCountVsTime;

        private bool _showStem = true;

        private bool _showLines;

        private MousePerformanceChartWindowPlacement _placement = new MousePerformanceChartWindowPlacement(false, 0.0, 0.0, 0.0, 0.0, false);

        public MousePerformancePreferences LoadPreferences()
        {
            return new MousePerformancePreferences(_lastCpi, _plotType, _showStem, _showLines, _placement);
        }
        ...
        {
            _placement = placement ?? new MousePerformanceChartWindowPlacement(false, 0.0, 0.0, 0.0, 0.0, false);
        }
    }
}
