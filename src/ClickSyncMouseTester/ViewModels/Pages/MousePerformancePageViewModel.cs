...
public class MousePerformancePageViewModel : BindableBase, IDisposable, ICaptureSessionPageViewModel, ICaptureKeyboardShortcutHandler, INavigationResettablePageViewModel
{
    ...
    public MousePerformancePageViewModel(IRawInputBroker rawInputBroker, IMousePerformancePreferencesStore preferencesStore = null)
        : this(rawInputBroker, preferencesStore, null)
    {
    }

    internal MousePerformancePageViewModel(IRawInputBroker rawInputBroker, IMousePerformancePreferencesStore preferencesStore, DeviceSelectionCoordinator deviceSelectionCoordinator)
    {
    ...
    private void RefreshDevices()
    {
        ...
        RawMouseDeviceInfo selectedDevice = ResolveDeviceSelectionAfterRefresh(previousSelectedId);
    ...
    private RawMouseDeviceInfo ResolveDeviceSelectionAfterRefresh(string previousSelectedId)
    {
        if (_deviceSelectionCoordinator != null)
        {
            return _deviceSelectionCoordinator.ResolveSelectionAfterRefresh(_devices, previousSelectedId);
        }

        return RawMouseDeviceSelectionPolicy.ResolveSelectionAfterRefresh(_devices, previousSelectedId);
    }

    private void ApplyPreferredDeviceSelection()
    {
        if (_deviceSelectionCoordinator == null || IsLocked || _sessionSourceMode == MousePerformanceSessionSourceMode.Imported)
        {
            return;
        }
    ...
    {
        return !IsLocked && _sessionSourceMode == MousePerformanceSessionSourceMode.Live && SelectedDevice != null;
    }
