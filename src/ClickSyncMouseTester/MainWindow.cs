...
public partial class MainWindow : Window
{
    ...
    private ICaptureKeyboardShortcutHandler _activeKeyboardShortcutPage;
    ...
    public MainWindow(ShellViewModel shellViewModel, bool usesStartupMenuHandoff)
    {
        ...
        AttachKeyboardShortcutPage(_shellViewModel.ActiveKeyboardShortcutPage);
    }
    ...
    private void AttachKeyboardShortcutPage(ICaptureKeyboardShortcutHandler nextPage)
    {
        _activeKeyboardShortcutPage = nextPage;
    }
    ...
    protected override void OnPreviewKeyDown(KeyEventArgs e)
    {
        ...
        else if (!_isCaptureLocked && CanHandleCaptureShortcut(e) && Keyboard.FocusedElement is not TextBox && _activeKeyboardShortcutPage != null)
        {
            e.Handled = _activeKeyboardShortcutPage.TryHandleCaptureKeyboardShortcut(CaptureKeyboardShortcut.StartOrPause);
        }
        else
        {
    ...
    private async void OnShellViewModelPropertyChanged(object sender, PropertyChangedEventArgs e)
    {
        ...
        if (isMenuOpening)
        {
            ...
            await PrewarmNavigationCurtainRenderAsync(menuItems);
            if (navigationMenuAnimationVersion != _navigationMenuAnimationVersion)
            {
                return;
            }
    ...
    private void PrepareStartupMenuHandoffState()
    {
        ...
        if (_shellViewModel != null && _shellViewModel.IsNavigationMenuOpen)
        {
            ...
            NavigationCurtainSurface.UpdateLayout();
            NavigationBrandLogoControl?.Preheat();
    ...
    private async Task PrewarmNavigationCurtainRenderAsync(IReadOnlyList<NavigationMenuAnimationItem> menuItems)
    {
        if (NavigationCurtainSurface == null)
        {
            return;
        ...
        double previousOpacity = NavigationCurtainSurface.Opacity;
        bool previousHitTestState = NavigationCurtainSurface.IsHitTestVisible;
        try
        {
            NavigationCurtainSurface.Opacity = 0.0;
            NavigationCurtainSurface.IsHitTestVisible = false;
            ...
            NavigationBrandLogoControl?.Preheat();
            await NavigationMotion.WaitForNextRenderAsync();
        }
        finally
        {
            NavigationCurtainSurface.Opacity = previousOpacity;
            NavigationCurtainSurface.IsHitTestVisible = previousHitTestState;
            UpdateNavigationCurtainClip(0.0);
        }
    }
